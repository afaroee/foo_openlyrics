...
public:
    ...
    PreferencesEdit(preferences_page_callback::ptr callback)
        : m_callback(callback)
    {
    }
...
{
    if((notify->idFrom != IDC_EDIT_AUTOEDITS_AUTOMATED) || (notify->code != LVN_ITEMCHANGED))
    {
        on_ui_interaction();
    }
...
{
    SendDlgItemMessage(IDC_EDIT_AUTOEDITS_AUTOMATED,
...
public:
    ...
    GUID get_guid() final
    {
        return GUID_PREFERENCES_PAGE_EDIT;
    }
